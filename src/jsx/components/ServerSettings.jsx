...
export default function ServerSettings({ WebSocketManager, onClose, onSave }) {
  ...
  const loadSettings = () => {
    ...
    if (settings) {
      if (settings.servers) {
        // Merge saved settings with defaults, handling version field
        const merged = defaultServers.map(server => {
          const saved = settings.servers.find(s => 
            s.type === server.type && 
            (s.version === server.version || (!s.version && !server.version))
          );
  ...
  const handleSaveSettings = () => {
    // Save with version field if it exists
    const settings = {
      servers: servers.map(s => {
        const serverConfig = { type: s.type, enabled: s.enabled };
        if (s.version) {
          serverConfig.version = s.version;
        }
        return serverConfig;
      }),
  ...
  const handleToggleServer = (index) => {
  ...
  const handleTestCloudConnection = async (server) => {
    ...
    try {
      const rooms = await WebSocketManager.fetchRoomsFromServer(
        server.url, 
        server.name,
        server.version || 'legacy'
      );
  ...
  const getServerBadge = (server) => {
    if (server.version === 'new') {
      return (
        <span className="text-[10px] px-2 py-0.5 bg-green-900 bg-opacity-30 text-green-400 rounded border border-green-700 border-opacity-50 uppercase font-bold ml-2">
          NEW
        </span>
      );
    } else if (server.version === 'legacy') {
      return (
        <span className="text-[10px] px-2 py-0.5 bg-amber-900 bg-opacity-30 text-amber-400 rounded border border-amber-700 border-opacity-50 uppercase font-bold ml-2">
          LEGACY
        </span>
      );
    }
        ...
        <div className="flex flex-col h-full p-6">
          ...
          <div className="flex-1 overflow-y-auto pr-2 scrollbar-custom min-h-0">
            <div className="space-y-6 pb-2">
              <div>
                ...
                <div className="space-y-3">
                  {servers.map((server, index) => (
                    <div key={`${server.type}-${server.version || 'default'}-${index}`} className="bg-zinc-800 rounded-lg p-4 border border-yellow-900">
              ...
              {/* Info box about server versions */}
              {servers.some(s => s.version) && (
                <div className="p-3 bg-zinc-800 bg-opacity-50 rounded border border-yellow-900">
                  <h4 className="text-yellow-400 font-bold mb-2 text-sm uppercase">ℹ️ Server Info</h4>
                  <ul className="text-xs text-gray-400 space-y-1.5">
                    <li>• <strong className="text-yellow-500">NEW:</strong> Latest server with bug fixes</li>
                    <li>• <strong className="text-yellow-500">LEGACY:</strong> Original server (fallback)</li>
                    <li>• Enable at least one server to play online</li>
                  </ul>
                </div>
              )}
            </div>
