...
type Rank = '5★' | '4★' | '3★' | '2★' | '1★' | 'COL' | 'LTC' | 'MAJ' | 'CPT' | '1LT' | '2LT' | 'SGT' | 'PVT' | 'SPY' | 'FLAG';
type Player = 1 | 2;
type Phase = 'setup' | 'playing';
type Mode = 'ai' | '2player' | null;

interface Piece { r: Rank; p: Player; id: number; }
type Board = (Piece | null)[][];

const RANKS: { r: Rank; count: number }[] = [
  { r: '5★', count: 1 }, { r: '4★', count: 1 }, { r: '3★', count: 1 }, { r: '2★', count: 1 }, { r: '1★', count: 1 },
...
const createPieces = (p: Player): Piece[] => {
  const pieces: Piece[] = [];
  let id = 0;
...
const initBoard = (): Board => Array(8).fill(null).map(() => Array(9).fill(null));

const autoSetup = (b: Board, p: Player): Board => {
...
const battle = (a: Piece, d: Piece): 'win' | 'lose' | 'draw' => {
...
const validMoves = (b: Board, r: number, c: number): [number, number][] => {
  const dirs = [[-1, 0], [1, 0], [0, -1], [0, 1]];
  return dirs.map(([dr, dc]) => [r + dr, c + dc] as [number, number])
    .filter(([nr, nc]) => nr >= 0 && nr < 8 && nc >= 0 && nc < 9 && (!b[nr][nc] || b[nr][nc]!.p !== b[r][c]!.p));
};

const aiMove = (b: Board): { from: [number, number]; to: [number, number] } | null => {
...
export default function Salpakan() {
  ...
  const [setupPlayer, setSetupPlayer] = useState<Player>(1);
  ...
  const startSetup = (m: Mode) => {
    ...
    setSetupPlayer(1);
    setPhase('setup');
    setMsg('Player 1: Place your pieces (drag or click)');
  };
  ...
  if (phase === 'setup' && inventory.length === 0 && setupPlayer === 1 && mode === 'ai') {
    return null;
  }
    ...
    <div className="flex flex-col items-center justify-center min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 p-4">
      ...
      {phase === 'setup' && inventory.length > 0 && (
        <div className="mt-4 p-4 bg-slate-800 rounded-lg">
          <div className="text-white mb-2">Remaining pieces:</div>
